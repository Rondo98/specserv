  ...
  try {
	...
	//$( document ).ready(function() {
		...
		Drupal.services.call({
			method: 'POST',
			path: 's_custom_service_resources/s_theme_settings.json',
			data: JSON.stringify({'site_id': drupalgap.settings.site_id}),
			success: function(result) {
				console.log(result);
				
				if (result.field_app_css) {
					$(result.field_app_css).appendTo( "head" )
				}
				else {
					$('body.s-dark-theme').css({'background': 'linear-gradient(to bottom, rgba(7,2,9,1) 0%,rgba(87,51,87,1) 50%,rgba(7,2,9,1) 100%)'});
				}
				
				if (result.field_app_logo) {
					$('.s-app-loading-page-wrapper .s-app-loading-logo-wrapper').html('<img src="'+result.field_app_logo+'" alt="" />'+(result.field_app_slogan ? '<span>'+result.field_app_slogan+'</span>' : ''));
				}
				else {
					$('.s-app-loading-page-wrapper .s-app-loading-logo-wrapper').html('<img src="images/logo.png" alt="" /><span>Слоган или название компании</span>');
				}
				
				if (result.field_app_logo_background) {
					$('body').addClass('s-app-bg-overlay-active');
					$('.s-app-bg-overlay').addClass('active').css({'background': 'url('+result.field_app_logo_background+') no-repeat center center'});
				}
				
				drupalgap.settings.app_settings = result;
...
function _drupalgap_deviceready() {
  try {
    ...
    //localStorage.removeItem('s_user_phone');
	  ...
	  var s_user_hash = localStorage.getItem('s_user_hash');
	  
	  if (!s_user_hash && drupalgap.settings.mode == 'phonegap') {
		  NativeStorage.getItem("s_user_hash",
			function (result) {
				localStorage.setItem('s_user_hash', result);
			},
			function (e) {
		...
		if (!s_user_phone) {
			if (drupalgap.settings.mode == 'web-app') {
				...
				var number = '+79184590028'.replace(/[^\d]+/g, "");
				var s_params = {
					's_phone': number,
              ...
              localStorage.setItem('s_user_hash', s_hash);
						  ...
						  if (!localStorage.getItem('s_user_date_app_registration')) {
							  localStorage.setItem('s_user_date_app_registration', '1565601539');
						  }
						  if (drupalgap.settings.mode == 'phonegap')
							_front_page_check_mobile_client(s_hash, localStorage.getItem('s_user_date_app_registration'), false);
						}
				});
		...
		// --- User registration timestamp --- //
			...
			if (!localStorage.getItem('s_user_date_app_registration')) {
				if (drupalgap.settings.mode == 'phonegap') {
					NativeStorage.getItem("s_user_date_app_registration",
						function (result) {
							localStorage.setItem('s_user_date_app_registration', result);
						},
						function (e) {
							var s_user_date_app_registration = Math.floor(Date.now() / 1000);
							
							NativeStorage.setItem("s_user_date_app_registration",s_user_date_app_registration,
							  function (result) {
								localStorage.setItem('s_user_date_app_registration', s_user_date_app_registration);
							  },
								function (e) {
									console.log("Read Object s_user_date_app_registration Failed");
							});
					});
				}
				//else {
					//var s_user_date_app_registration = localStorage.getItem('s_user_date_app_registration');
					//if (!s_user_date_app_registration) {
						//localStorage.setItem('s_user_date_app_registration', '1565595984');
					//}
				//}
			}
  ...
  try {
    ...
    return {
      success: function(result) {
        ...
        // Segaweb
        if (drupalgap.settings.mode == 'phonegap')
			_front_page_check_mobile_client(localStorage.getItem('s_user_hash'), 'date_registration', false);
        // END Segaweb
...
function drupalgap_services_preprocess(options) {
  ...
  if (options.path && options.path == 's_custom_service_resources/s_chat_update_worker.json') return;
...
function drupalgap_back_2tf() {
  ...
  drupalgap_remove_page_from_dom(drupalgap_get_page_id(back), { force: true });
      ...
      if (
        ...
        if (
          ...
          region.collapse_on_empty === false
			...
			region_html += '<div class="s-header-content">' +
								'<div class="s-company-name">Фавикон и название вашей компании</div>' +
								'<div class="s-company-logo">'+theme('image', {path: drupalgap.settings.logo, attributes: {class: 's-app-logo'}})+'</div>' +
							'</div>'; 
		  }
      }
        ...
        if (element.required) {
          ...
          if (!valid) {
            ...
            drupalgap_form_set_error(
              ...
              t('Поле') + ' "' + field_title + '" ' + t('обязательно к заполнению') + '.'
            );
          }
        }
    }
...
function hook_form_alter(form, form_state, form_id) {
  ...
  if (form_id == 'user_login_form') {
    ...
    form.elements['name'].description = 'Введите логин';
  }
}
        ...
        confirmCallback: function(button) {
          ...
          call_arguments.success = function(result) {
            ...
            if (drupalgap_page_in_dom(entity_page_id)) {
              drupalgap_remove_page_from_dom(entity_page_id, { force: true });
            }
            ...
            try {
				form.action = form.buttons.delete.attributes.s_del_redirect;
			}
			catch(error) {}
            //console.log('form');
            //console.log(form);
...
function drupalgap_entity_render_field(entity_type, entity, field_name,
  ...
  try {
    ...
    if (content != '' && display['label'] != 'hidden') {
      ...
      label = '<div class="s-label">' + label + '</div>';
 ...
 * @param {Object} build
 */
function _drupalgap_entity_page_container_inject(entity_type, entity_id, mode, build, entity = false) {
  try {
    ...
    _drupalgap_entity_page_add_css_class_names(entity_type, entity_id, build, entity);
  }
  catch (error) {
 ...
 * @private
 */
function _drupalgap_entity_page_add_css_class_names(entity_type, entity_id, build, entity = false) {
  try {
    var className = entity_type;
    if (entity) {
		var bundleName = entity_get_bundle(entity_type, entity);
	}
	else {
    ...
    node_load(nid, {
        success: function(node) {
          ...
          if (modules.length > 0) {
            ...
            var fn = window[function_name];
            //console.log(node.s_html);
            fn(node, {
                success: function(content) {
                  _drupalgap_entity_page_container_inject(
                    'node', node.nid, 'view', content, node
                  );
                }
            });
            return;
  ...
  try {
    switch (delta) {
      ...
      case 'title':
        ...
        break;
      case 'powered_by':
        return '<p style="text-align: center;">'+l('<i class="fi_ofctr fi-logo_footer"></i>', 'https://office-controller.ru', {'attributes': {'class':'s-footer-logo'},InAppBrowser: true})+'</p>';
        break;
      case 'help':
...
function entity_get_bundle(entity_type, entity) {
  try {
    ...
    var bundle = null;
    switch (entity_type) {
      case 'node': bundle = entity.type.replace(/_/g, '-'); break;
