...
$.widget("mobile.date",{
	...
	_create:function(){
		...
		if (this.element.data('mindate'))
			this.options.minDate = this.element.data('mindate');

		if( this.options.inline ){
			this.options.altField = this.element;
			calendar = $("<div>").datepicker(this.options);
			if (this.element.parent().hasClass('form-item')) {
				this.element.after(calendar);
			}
			else this.element.parent().after(calendar);
		} else {
