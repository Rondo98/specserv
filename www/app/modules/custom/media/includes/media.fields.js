...
function media_field_widget_form(form, form_state, field, instance, langcode,
  ...
  try {
    ...
    if (typeof items[delta].item !== 'undefined' && items[delta].item.fid) {
      ...
      if (form.arguments[delta][field.field_name][langcode].length > 1) {
			for (var s_item = 1; s_item < form.arguments[delta][field.field_name][langcode].length; s_item++) {
				
				var s_image = {
				  path:drupalgap_image_path(form.arguments[delta][field.field_name][langcode][s_item]['uri']),
				};
				var s_html = theme('image', s_image);
				console.log(s_html);
				media += s_html;
			}
		}
    ...
    var html = '<div id="' + media_field_widget_media_containter_id(items[delta].id) + '"></div>' +
      ...
      html += '<div class="drupalgap_form_add_another_item"></div>';
